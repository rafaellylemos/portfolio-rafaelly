"use client"; // Essencial para o scroll suave e eventos de clique
    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-background">
      ...
      <div className="relative z-10 container max-w-6xl mx-auto px-6 text-center">
        ...
        <h1 className="font-heading text-5xl md:text-7xl lg:text-8xl font-semibold text-foreground mb-6 tracking-tight">
          Frontend Developer
          <br />
          <span className="text-accent">&</span> Product Thinker
        </h1>
        ...
        <p className="font-body text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto mb-12">
          ...
          Especialista em <span className="text-foreground font-medium">React</span> e <span className="text-foreground font-medium">Next.js</span> com visão estratégica de produto.
        </p>

        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-16">
          <a
            href="mailto:rafaellyltbmarques@gmail.com"
            className="px-8 py-4 bg-foreground text-background text-base font-medium rounded-full hover:opacity-90 transition-all hover:scale-105 active:scale-95"
          >
            Iniciar Conversa
          </a>
          <button
            onClick={() => document.querySelector("#experience")?.scrollIntoView({ behavior: "smooth" })}
            className="px-8 py-4 border border-border text-foreground text-base font-medium rounded-full hover:bg-accent/5 transition-all"
          >
            Ver Experiência
          </button>
        </div>
